 ...
 * @returns {Promise<Object>} Response object
 */
export const fetchHistoricalData = async (stockSymbol, actual, from, to) => {
  const url = `${basePath}/stock/earnings?symbol=${stockSymbol}&actual=${actual}&from=${from}&to=${to}&token=${process.env.REACT_APP_API_KEY}`;
  const response = await fetch(url);
...
// export const fetchEarningsSurprises = async (stockSymbol, limit) => {
//   const url = `${basePath}/stock/earnings?symbol=${stockSymbol}${
//     limit ? `&limit=${limit}` : ""
